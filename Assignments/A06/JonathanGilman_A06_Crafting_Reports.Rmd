...
output:
  pdf_document:
    ...
    fig_caption: true       # enables figure captions
...
```{r project.setup, message=FALSE, warning=FALSE, error=FALSE, echo=TRUE, results="hide"}
# Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
# table for PM2.5
knitr::kable(summary_PM25, col.names = c("County", "$\\mu g/m^3$"), caption = "Mean Particulates (2.5mm)")
...
```{r ozone_2018, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.align='left', fig.height=4, fig.cap='Ozone Levels by Month in 2018', fig.pos='!h'}
# Filter data for 2018
ozone_2018 <- epa_data %>% filter(year(Date) == 2018)

# Create boxplot for Ozone levels by month in 2018
ggplot(ozone_2018, aes(x = factor(month(Date), levels = 1:12), y = Ozone)) +
  geom_boxplot() +
...
```markdown
\clearpage
```{r ozone_2019, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.align='left', fig.height=4, fig.cap='Ozone Levels by Month in 2019', fig.pos='!h'}
# Filter data for 2019
ozone_2019 <- epa_data %>% filter(year(Date) == 2019)

# Create boxplot for Ozone levels by month in 2019
ggplot(ozone_2019, aes(x = factor(month(Date), levels = 1:12), y = Ozone)) +
  geom_boxplot() +
...
Link to GitHub repository: https://github.com/jonathangilman/EDE_Fall2024.git
